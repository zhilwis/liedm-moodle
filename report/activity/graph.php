 ...
 * @package    report
 * @subpackage activity
 * @copyright  1999 onwards Martin Dougiamas (http://dougiamas.com) (modification)
...
$url = new moodle_url('/report/activity/graph.php', array('course'=>$courseid, 'report'=>$report, 'time'=>$time, 'mode'=>$mode, 'userid'=>$userid, 'roleid'=>$roleid));
$PAGE->set_url($url);
...
if (!empty($userid)) {
    ...
    if (!report_activity_can_access_user_report($user, $course, true)) {
        require_capability('report/activity:view', $coursecontext);
    }
...
// if(!empty($groupname)){add_to_log($course->id, 'course', 'report activity', "report/activity/graph.php?userid=$userid&id=$course->id&mode=$mode&roleid=$roleid&group=$groupname", $course->id);}
// else{
	// add_to_log($course->id, 'course', 'report activity', "report/activity/graph.php?userid=$userid&id=$course->id&mode=$mode&roleid=$roleid", $course->id);
// }
add_to_log($course->id, 'course', 'report activity', "report/activity/graph.php?userid=$userid&id=$course->id&mode=$mode&roleid=$roleid", $course->id);
...
// echo $param->table ;
//TODO: cleanup this ugly mess! i absolutely agree on this :) This is bullshit
...
// $sql = 'SELECT '.((empty($param->fieldscomplete)) ? 'id,
													// roleid,
													// timeend,' : '').$param->fields
			// .' FROM {stats_'.$param->table.'} '
			// .((!empty($groupname)) ? ' Inner Join mdl_user ON mdl_stats_user_monthly.userid = mdl_user.id ': '')
// 				
			// .' WHERE '
			// .(($course->id == SITEID) ? '' : ' courseid = '.$course->id.' AND ')
			// .((!empty($userid)) ? ' userid = '.$userid.' AND ' : '')
			...
			. $param -> fields . ' FROM {stats_' . $param -> table . '} WHERE ' 
			. (($course -> id == SITEID) ? '' : ' courseid = ' . $course -> id . ' AND ') 
			. ((!empty($userid)) ? ' userid = ' . $userid . ' AND ' : '') 
			. ((!empty($roleid)) ? ' roleid = ' . $roleid . ' AND ' : '')
			. ((!empty($param -> stattype)) ? ' stattype = \'' . $param -> stattype . '\' AND ' : '') 
